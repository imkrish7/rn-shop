    ...
    return useQuery({
        ...
        queryFn: async()=>{
            ...
            if(products.error || categories.error){
                throw new Error("An error occured while fetching products and categories")
            }
...
export const getProduct = (slug: string)=>{
    return useQuery({
        ...
        queryFn: async ()=>{
            ...
            if(error || !data){
                throw new Error('Error while fetching the products:'+error?.message);
            }
  ...
  return useMutation({
    async mutationFn(insertData: { 
      ...
      quantity: number
        ...
        const { data, error } = await supabase.from('order_item')
        .insert(insertData.map(({orderId, quantity, productId})=> ({
          order: orderId,
          product: productId,
          quantity
        }))
      ...
      const productQuantities = insertData.reduce((acc, item)=> {
      ...
      if(error){
        throw new Error('An error occured while creating an order'+error?.message)
      }
      return data;
    }
  })
